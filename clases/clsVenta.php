...
public function generarNumVenta() {
	...
	return "V".$strNum_venta;

}


//Guardar los datos de la Venta 
function guardarFactura($productos,$idtipodoc,$idclie,$idemp,$serie,$numfact,$valorventa,$igv,$total,$estado){
	$con = new clsConexion;
	if($con->conectarse()==true){
		$query="CALL SP_I_Venta($idtipodoc,$idclie,$idemp,'$serie','$numfact',now(),$valorventa,$igv,$total,'$estado')";
		$result = @mysql_query($query);				 
		if(!$result){
