...
class clsPedido
{
 ...
 //Modificar empleado en la base de datos
   ...
   //usamos el metodo conectar para realizar la conexion
	...
	function guardarPedido($productos,$idcliente,$npedido,$valorventa,$igv,$total,$estado)
	{
		$con = new clsConexion;
		if($con->conectarse()==true)
		{
			$query = "CALL SP_I_Pedido('$idcliente','$npedido',now(), ' ', '$valorventa','$igv','$total', '$estado')";
			$result = @mysql_query($query);				 
			if(!$result)
			{
